...
class Node {
  ...
  class SinglyLinkedList {
    ...
    //TODO
    /**
     * Concatenates the nodes of a given list into the back of this list.
     * - Time: O(?).
     * - Space: O(?).
  ...
  // Testing concat, moveMinToFront, and splitOnVal functions
  const list1 = new SinglyLinkedList();
  list1.insertAtBackMany([1, 3, 5, 7, 9]);
  
  const list2 = new SinglyLinkedList();
  list2.insertAtBackMany([2, 4, 6, 8]);
  
  console.log("List1 before concat:", list1.toArr());
  console.log("List2 before concat:", list2.toArr());
  
  // Test concat
  list1.concat(list2);
  console.log("List1 after concat with List2:", list1.toArr()); // Expected: [1, 3, 5, 7, 9, 2, 4, 6, 8]
  console.log("List2 remains unchanged:", list2.toArr()); // Expected: [2, 4, 6, 8]
  ...
  console.log("List4 after moveMinToFront:", list4.toArr()); // Expected: [5, 25, 12, 30, 20]
  
  // Test splitOnVal
  const list5 = new SinglyLinkedList();
  list5.insertAtBackMany([1, 2, 3, 4, 5, 6]);
  console.log("List5 before splitOnVal(4):", list5.toArr());
  const splitList = list5.splitOnVal(4);
  console.log("List5 after splitOnVal(4):", list5.toArr()); // Expected: [1, 2, 3]
  console.log("New list from splitOnVal(4):", splitList.toArr()); // Expected: [4, 5, 6]
  
  // Edge cases
  
  // Empty list for moveMinFront, moveMinToFront, and splitOnVal
  const emptyList = new SinglyLinkedList();
  console.log("Empty list before operations:", emptyList.toArr());
  emptyList.moveMinToFront();
  console.log("Empty list after moveMinToFront:", emptyList.toArr()); // Expected: []
  const splitEmpty = emptyList.splitOnVal(10);
  console.log("Split empty list on value 10:", splitEmpty.toArr()); // Expected: []
  
  // Split on value not in the list
  const list6 = new SinglyLinkedList();
  list6.insertAtBackMany([10, 20, 30, 40]);
  console.log("List6 before splitOnVal(50):", list6.toArr());
  const noSplit = list6.splitOnVal(50);
  console.log("List6 after splitOnVal(50):", list6.toArr()); // Expected: [10, 20, 30, 40]
  console.log("No split list when value 50 not found:", noSplit.toArr()); // Expected: []
  
  // Move minimum in a single-node list
  const singleNodeList = new SinglyLinkedList();
  singleNodeList.insertAtFront(10);
  console.log("Single node list before moveMinFront:", singleNodeList.toArr());
  singleNodeList.moveMinToFront();
  console.log("Single node list after moveMinToFront:", singleNodeList.toArr()); // Expected: [10]
